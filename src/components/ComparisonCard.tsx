...
export const ComparisonCard: React.FC<ComparisonCardProps> = ({ comparison }) => {
  return (
    <div className="comparison-card bg-white border-2 border-indigo-200 rounded-lg p-6 mb-6">
      {/* Title */}
          ...
          <tbody>
            {comparison.options.map((option, i) => (
              <tr
                key={i}
                className={`border-b ${
                  option.chosen ? 'bg-indigo-50' : 'bg-white'
                }`}
              >
                <td className="px-4 py-4">
                  <span className="font-semibold">{option.name}</span>
                  {option.chosen && (
                    <span className="ml-2 inline-block bg-indigo-600 text-white text-xs px-2 py-1 rounded">
                      ✓ Chosen
                    </span>
                  )}
                </td>
      ...
      <div className="bg-indigo-50 border-l-4 border-indigo-500 p-4 rounded">
        <h4 className="font-semibold text-indigo-900 mb-2">Why We Chose This</h4>
